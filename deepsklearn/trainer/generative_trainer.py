...
class GenerativeTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            ...
            for feature_dict,label_dict in self.train_dataloader:
                ...
                labels = label_dict["label"].to(self.device) #(batch_size,seq_len)
                step_size = labels.shape[0]
                labels=torch.reshape(labels,(-1,))
                logits=model(feature_dict) # (batch_size,seq_len,num_items+1)
    ...
    def _evaluation(self,*,epoch,model_name):
        ...
        for feature_dict,label_dict in self.validation_dataloader:
            ...
            step_size=labels.shape[0]
            labels = torch.reshape(labels, (-1,))
            logits = self.model(feature_dict)  # (batch_size,seq_len,num_items+1)
            logits = torch.reshape(logits, (-1, logits.shape[-1]))
