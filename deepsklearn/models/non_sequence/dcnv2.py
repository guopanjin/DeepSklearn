...
class CrossNetworkV2(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layer):
            layer_list.append(nn.Linear(self.feature_size, self.feature_size, bias=True))
...
class DCNV2(nn.Module):
    def __init__(self,
        ...
        self.embedding_dict = nn.ModuleDict(
            {feature_name: nn.Embedding(embedding_number, embedding_dim=self.embedding_size) for
             feature_name, embedding_number in self.feature_columns.items()})
        ...
        self.cross_layer = CrossNetworkV2(feature_size=self.feature_size * self.embedding_size,
