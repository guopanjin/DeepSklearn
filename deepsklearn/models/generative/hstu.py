...
from deepsklearn.layers.hstu_layer import HSTULayer
...
2: 
  ...
  x=self.hstu_layer(x,padding_mask=padding_mask)# 2 layers  (batch_size,seq_len,embed_dim)
...
class HSTU(nn.Module):
    def __init__(self,
        ...
        self.num_hstu_layers=num_hstu_layers

        self.item_embedding_table=nn.Embedding(self.num_items+1,self.embed_dim,padding_idx=0)
        hstu_layer_list=[]
        for i in  range(self.num_hstu_layers):
            hstu_layer=HSTULayer(embed_dim=self.embed_dim,
                                      num_heads=self.num_heads,
                                      max_len=self.seq_len
                                      )
            hstu_layer_list.append(hstu_layer)
        self.hstu_layer_module_list=nn.ModuleList(hstu_layer_list)
    ...
    def forward(self,x):
        ...
        for hstu_layer in self.hstu_layer_module_list:
            x=hstu_layer(x,padding_mask=padding_mask)
